...
export function WhatsappConfig() {
  ...
  async function carregarConfiguracao(): Promise<void> {
    ...
    setAtivo(resultado.dados.ativo);
    setNumeroAviso(resultado.dados.numeroAviso ?? "");
    setMensagemPrimeiroContato(resultado.dados.mensagemPrimeiroContato);
  }
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto rounded-[28px] bg-snow p-5 sm:max-w-3xl">
          ...
          {carregando ? (
            ...
            <form onSubmit={(evento) => void salvarConfiguracao(evento)} className="grid gap-4">
              ...
              {!configuracao?.apiHabilitada ? (
                <div className="rounded-[24px] bg-fog p-4 text-sm leading-6 text-steel">
                  A Evolution está desabilitada no ambiente. Preencha `EVOLUTION_API_HABILITADA`, URL, chave e webhook no
                  `.env` para gerar QR Code real.
                </div>
              ) : null}
              ...
              <div className="grid gap-4 md:grid-cols-[1fr_220px]">
                <div className="grid gap-4 rounded-[24px] bg-mist p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-steel">
                      A mensagem deve conter referências a nome, assunto e descrição.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid content-start gap-3 rounded-[24px] bg-mist p-4 text-sm text-steel">
                  ...
                  <Button
                    ...
                    disabled={!configuracao?.apiHabilitada}
